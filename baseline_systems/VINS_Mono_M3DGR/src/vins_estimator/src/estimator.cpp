...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        // 输出当前帧的特征点匹配数量
        ROS_INFO("Frame %d: Correspondences size = %lu", i, corres.size());
        ...
        if (corres.size() > 20)
        {
            ...
            // 输出计算的平均视差
            ROS_INFO("Frame %d: Average parallax = %f", i, average_parallax * 460);
            ...
            {
                ROS_INFO("Frame %d: Parallax is sufficient", i);

                if (m_estimator.solveRelativeRT(corres, relative_R, relative_T))
                {
                    l = i;
                    ROS_INFO("Frame %d: Successfully solved relative pose", i);
                    ROS_INFO("average_parallax %f choose l %d and newest frame to triangulate the whole structure", average_parallax * 460, l);
                    return true;
                }
                else
                {
                    ROS_INFO("Frame %d: Failed to solve relative pose", i);
                }
            }
            else
            {
                ROS_INFO("Frame %d: Parallax is insufficient (average_parallax * 460 = %f)", i, average_parallax * 460);
            }
        }
        else
        {
            ROS_INFO("Frame %d: Not enough correspondences (size = %lu)", i, corres.size());
        }
    }
    return false;
}
